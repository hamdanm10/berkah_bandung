...
class SuperAdmin::ReturnedOrdersController < SuperAdminApplicationController
  ...
  def create
    ...
    render turbo_stream: [
      ...
      turbo_stream.replace(
        'return_results',
        partial: 'super_admin/returned_orders/return_results',
        locals: { order: order }
      )
    ]
  end
  ...
  def undo
    ...
    if result.success?
      redirect_to super_admin_returned_orders_path, notice: result.payload[:message]
    else
      redirect_to super_admin_returned_orders_path, alert: result.error[:order]
    end
  end
