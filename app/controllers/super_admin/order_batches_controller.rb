...
class SuperAdmin::OrderBatchesController < SuperAdminApplicationController
  def index
  ...
  def create
    ...
    if result.success?
      redirect_to new_super_admin_merchant_order_order_batch_path(@merchant), notice: result.payload[:message]
    else
