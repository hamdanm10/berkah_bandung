...
class SuperAdmin::AccountSettingsController < SuperAdminApplicationController
  def edit
    @user = current_user
  end
  ...
  def update
    ...
    if result.success?
      redirect_to super_admin_account_setting_path, notice: result.payload[:message]
    else
  ...
  def change_password
    ...
    if result.success?
      redirect_to super_admin_account_setting_path,
