...
class SuperAdmin::CancelledOrdersController < SuperAdminApplicationController
  ...
  def create
    ...
    respond_to do |format|
      format.turbo_stream
      format.html { redirect_to new_super_admin_cancelled_order_path }
    end
  end
  ...
  def undo
    ...
    if result.success?
      redirect_to super_admin_cancelled_orders_path, notice: result.payload[:message]
    else
      redirect_to super_admin_cancelled_orders_path, alert: result.error[:order]
    end
  end
