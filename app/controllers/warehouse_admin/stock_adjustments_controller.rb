...
class WarehouseAdmin::StockAdjustmentsController < WarehouseAdminApplicationController
  def index
  ...
  def create
    ...
    if result.success?
      redirect_to new_warehouse_admin_product_stock_adjustment_path(@product), notice: result.payload[:message]
    else
  ...
  def update
    ...
    if result.success?
      ...
      redirect_to edit_warehouse_admin_product_stock_adjustment_path(@product, product_available),
