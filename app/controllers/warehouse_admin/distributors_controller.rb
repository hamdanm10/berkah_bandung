...
class WarehouseAdmin::DistributorsController < WarehouseAdminApplicationController
  def index
  ...
  def create
    ...
    if result.success?
      redirect_to new_warehouse_admin_distributor_path, notice: result.payload[:message]
    else
  ...
  def update
    ...
    if result.success?
      ...
      redirect_to edit_warehouse_admin_distributor_path(distributor), notice: result.payload[:message]
    else
      @distributor = result.error[:distributor]
  ...
  def destroy
    ...
    if result.success?
      redirect_to warehouse_admin_distributors_path, notice: result.payload[:message]
    else
      redirect_to warehouse_admin_distributors_path, alert: result.error[:distributor]
    end
  end
