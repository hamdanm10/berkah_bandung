...
class WarehouseAdmin::ProductsController < WarehouseAdminApplicationController
  def index
  ...
  def create
    ...
    if result.success?
      redirect_to new_warehouse_admin_product_path, notice: result.payload[:message]
    else
  ...
  def update
    ...
    if result.success?
      ...
      redirect_to edit_warehouse_admin_product_path(product), notice: result.payload[:message]
    else
      @product = result.error[:product]
      assign_dropdown_search_labels(@product)
  ...
  def destroy
    ...
    if result.success?
      redirect_to warehouse_admin_products_path, notice: result.payload[:message]
    else
      redirect_to warehouse_admin_products_path, alert: result.error[:product]
    end
  end
  ...
  #   if result.success?
  #     redirect_to import_warehouse_admin_products_path, notice: result.payload[:message]
  #   else
