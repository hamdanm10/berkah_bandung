...
class WarehouseAdmin::RestockInvoicesController < WarehouseAdminApplicationController
  def index
  ...
  def create
    ...
    if result.success?
      redirect_to new_warehouse_admin_restock_invoice_path, notice: result.payload[:message]
    else
  ...
  def update
    ...
    if result.success?
      ...
      redirect_to edit_warehouse_admin_restock_invoice_path(invoice), notice: result.payload[:message]
    else
      @invoice = result.error[:invoice]
      assign_dropdown_search_labels(@invoice)
  ...
  def destroy
    ...
    if result.success?
      redirect_to warehouse_admin_restock_invoices_path, notice: result.payload[:message]
    else
      redirect_to warehouse_admin_restock_invoices_path, alert: result.error[:invoice]
    end
  end
