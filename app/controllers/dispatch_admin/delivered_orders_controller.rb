...
class DispatchAdmin::DeliveredOrdersController < DispatchAdminApplicationController
  def index
  ...
  def create
    ...
    respond_to do |format|
      format.turbo_stream
      format.html { redirect_to new_dispatch_admin_delivered_order_path }
    end
  end
  ...
  def undo
    ...
    if result.success?
      redirect_to dispatch_admin_delivered_orders_path, notice: result.payload[:message]
    else
      redirect_to dispatch_admin_delivered_orders_path, alert: result.error[:order]
    end
  end
