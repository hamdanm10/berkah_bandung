...
class ReturnAdmin::AccountSettingsController < ReturnAdminApplicationController
  def edit
    @user = current_user
  end
  ...
  def update
    ...
    if result.success?
      redirect_to return_admin_account_setting_path, notice: result.payload[:message]
    else
  ...
  def change_password
    ...
    if result.success?
      redirect_to return_admin_account_setting_path,
