...
class ReturnAdmin::CancelledOrdersController < ReturnAdminApplicationController
  def index
  ...
  def create
    ...
    respond_to do |format|
      format.turbo_stream
      format.html { redirect_to new_return_admin_cancelled_order_path }
    end
  end

  def undo
    result = Orders::UndoCancelOrder.call(
      order: order_scope
    )

    if result.success?
      redirect_to return_admin_cancelled_orders_path, notice: result.payload[:message]
    else
      redirect_to return_admin_cancelled_orders_path, alert: result.error[:order]
    end
  end
