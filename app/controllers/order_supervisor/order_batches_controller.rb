...
class OrderSupervisor::OrderBatchesController < OrderSupervisorApplicationController
  def index
    ...
    @q = @merchant
         .order_batches
         .where(created_by_user_id: Current.user.id, deleted_at: nil)
         .ransack(params[:q])
  ...
  def create
    ...
    if result.success?
      redirect_to new_order_supervisor_merchant_order_order_batch_path(@merchant), notice: result.payload[:message]
    else
  ...
  def update
    ...
    if result.success?
      ...
      redirect_to edit_order_supervisor_merchant_order_order_batch_path(@merchant, order_batch),
  ...
  def destroy
    ...
    if result.success?
      redirect_to order_supervisor_merchant_order_order_batches_path(@merchant), notice: result.payload[:message]
    else
      redirect_to order_supervisor_merchant_order_order_batches_path(@merchant), alert: result.error[:order_batch]
    end
  end
