...
class OrderSupervisor::OrdersController < OrderSupervisorApplicationController
  def index
  ...
  def create
    ...
    if result.success?
      redirect_to new_order_supervisor_merchant_order_order_batch_order_path(@merchant, @order_batch),
  ...
  def update
    ...
    if result.success?
      ...
      redirect_to edit_order_supervisor_merchant_order_order_batch_order_path(@merchant, @order_batch, order),
                  notice: result.payload[:message]
    else
      @order = result.error[:order]
      assign_dropdown_search_labels(@order)
  ...
  def destroy
    ...
    if result.success?
      redirect_to order_supervisor_merchant_order_order_batch_orders_path(@merchant, @order_batch),
                  notice: result.payload[:message]
    else
      redirect_to order_supervisor_merchant_order_order_batch_orders_path(@merchant, @order_batch),
  ...
  def duplicate_create
    ...
    if result.success?
      redirect_to duplicate_new_order_supervisor_merchant_order_order_batch_orders_path(@merchant, @order_batch),
                  notice: result.payload[:message]
    else
